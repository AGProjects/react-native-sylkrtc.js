...
class Addressbook extends EventEmitter {
    ...
    _populate(addressbookData) {
        // A populate carries a FULL addressbook snapshot, so replace the caches
        // wholesale. Merging (set without clearing) left items that had been
        // deleted on another device lingering forever — a re-fetch could never
        // drop a removed contact/group/policy, so deleted groups kept showing.
    ...
    _update(addressbookData) {
        if (addressbookData.action === 'delete') {
            // Drop the deleted item from the cache too — emitting the event
            // alone left the stale entry in the map (e.g. a deleted group kept
            // appearing until app restart).
            const _type = addressbookData.type;
            const _item = addressbookData[_type];
            if (_item && _item.id) {
                const _map = _type === 'contact' ? this._contacts
                    : _type === 'group' ? this._groups
                    : _type === 'policy' ? this._policies : null;
                if (_map) _map.delete(_item.id);
            }
    ...
    _updateFailed(errorData) {
        const { error, action, type, id, retryable } = errorData;
        this.emit('dataUpdateFailed', { error, action, type, id, retryable });
    }
