...
class Call extends EventEmitter {
    ...
    sendDtmf(tones, duration=100, interToneGap=70) {
        ...
        if (true) {
            ...
            this._pc.sendDTMF(tones, duration, interToneGap);
        }
    }
        ...
        switch (message.event) {
            case 'state':
                ...
                if (newState === 'accepted' || newState === 'early-media' && this._direction === 'outgoing') {
                    ...
                    let headers = Object.keys(message.headers).map(key => {return {name: key, value: message.headers[key]}});
                    this.emit('stateChanged', oldState, newState, {id: this._id, headers: headers});
