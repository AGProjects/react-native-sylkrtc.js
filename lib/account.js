...
class Account extends EventEmitter {
    ...
    syncConversations(id=null, options={} , cb=null) {
        const req = {
        ...
        switch (message.event) {
            ...
            case 'message':
                ...
                if (!incomingMessage) {
                    ...
                    // sylk-message-metadata (location share ticks, meeting
                    // handshakes, rotation/label/reply markers) are transient
                    // control messages — the sender doesn't need a wire
                    // delivery receipt for every tick, and one IMDN per
                    // location refresh (every ~60s) would bloat the archive.
                    // Skip the auto 'delivered' IMDN for these; the app layer
                    // still stamps received state locally and explicitly sends
                    // a receipt for location ORIGIN ticks where it matters.
