...
class Statistics extends EventEmitter {
    constructor(options={}) {
        ...
        // In-flight guard state for the monitoring poller — prevents
        // overlapping getStats calls from piling up pending native
        // callbacks. See _startStatsMonitoring.
        this._statsInFlight = false;
        this._statsInFlightSince = 0;
        this._statsStaleMs = Math.max(5000, this._getStatsInterval * 4);
    ...
    _startStatsMonitoring () {
        ...
        this._monitoringSetInterval = window.setInterval(() => {
            ...
            // In-flight guard: react-native-webrtc's getStats resolves
            // on the native bridge and can take longer than this poll
            // interval under load (a video call with several tracks is
            // the usual trigger). Without a guard each tick stacks a new
            // pending native callback on top of the unresolved ones, and
            // they accumulate until React Native warns "Excessive number
            // of pending callbacks". Skip the tick while a previous
            // _getStats() is still pending.
            //
            // Stale-timeout escape: if a getStats promise is genuinely
            // lost (e.g. the peer connection was torn down mid-call
            // during a renegotiation and native never calls back), don't
            // wedge the poller forever — after _statsStaleMs re-arm.
            if (this._statsInFlight
                    && (Date.now() - this._statsInFlightSince) < this._statsStaleMs) {
                return;
            }

            this._statsInFlight = true;
            this._statsInFlightSince = Date.now();
            this._getStats().then((statsEvents) => {
                statsEvents.forEach((statsEventObject) => {
                    this.emit(statsEventObject.tag, statsEventObject);
                });
            }).catch(() => {
                // A rejected getStats (e.g. closed PC) must not crash the
                // poller; the next tick re-evaluates.
            }).then(() => {
                this._statsInFlight = false;
            });
